...
void saveSbSImage(Camera& zed, std::string filename) {
    ...
    cv::Mat sbs_image(int(image_size.height), int(image_size.width) * 2, CV_8UC4);
    cv::Mat left_image(sbs_image, cv::Rect(0, 0, int(image_size.width), int(image_size.height)));
    cv::Mat right_image(sbs_image, cv::Rect(int(image_size.width), 0, int(image_size.width), int(image_size.height)));

    Mat buffer_sl;
    cv::Mat buffer_cv;

    zed.retrieveImage(buffer_sl, VIEW_LEFT);
    buffer_cv = cv::Mat(int(buffer_sl.getHeight()), int(buffer_sl.getWidth()), CV_8UC4, buffer_sl.getPtr<sl::uchar1>(MEM_CPU));
